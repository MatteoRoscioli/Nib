...
Overview
Nib OS is a lightweight operating system that runs directly on Raspberry Pi hardware. It features:

Direct Hardware Access - Runs directly on hardware
...
Nib>
Available Commands
CommandDescriptionExamplehelpDisplay all available commandshelplsList files on SD cardlscat <file>Display contents of a filecat hello.pyrun <file>Run a Python script (requires MicroPython)run test.pyecho <text>Echo text back to consoleecho Hello WorldclearClear the screenclearinfoShow system informationinfomemDisplay memory usage statisticsmemrebootReboot the systemreboot
Using Nib OS
Listing Files
Nib> ls
...
bash# In your workspace directory
...
Copy MicroPython source files to your Nib OS directory
Add MicroPython sources to the Makefile
Implement the MicroPython port interface
...
pythonprint("Hello from MicroPython!")
for i in range(5):
    print("Count:", i)
Then in Nib OS:
Nib> run hello.py
...
Built with ARM programming techniques and MicroPython.
Support
For issues, questions, or contributions, visit [your-github-repo-url]

Nib OS - Lightweight computing for Raspberry Pi
